...
Exports every open Safari tab (all windows) into a clean, self-contained
HTML page and a Markdown file, with an AI-written 1–2 sentence description
of what each page is — perfect for AirDropping your whole browsing session
to another machine.
...
- `Safari Tabs 2026-08-23 14.30.html` — a styled, clickable list (light/dark
  mode aware, fully self-contained). Open it on any device.
- `Safari Tabs 2026-08-23 14.30.md` — the same list as Markdown for notes apps.
...
- macOS with Safari (the tool reads tabs via AppleScript — nothing to install;
  it uses only the Python 3 that ships with macOS developer tools).
...
Safari — click **Allow** (it's under System Settings → Privacy & Security →
Automation if you need to change it later).
...
1. Asks Safari (via `osascript` / JXA) for the URL and title of every tab in
   every window.
