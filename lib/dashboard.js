...
<div class="card">
  <div class="bar"><span class="t">Models · five strategies, one book — nightly ensemble tune re-splits risk</span></div>
...
function render(){
  ...
  for(const p of poss){
    ...
    tr.innerHTML="<td class=sym>"+p.sym+"</td><td>"+(p.strat||"—")+"</td><td>"+(p.qty!=null?p.qty:"")+"</td>"+
      ...
      "<td>"+(p.scaled?"runner · trailing":(p.target!=null?"$"+Number(p.target).toFixed(2)+" · sell "+(p.scaleOutPct||85)+"%":(p.ridePct?"ride · exit −"+p.ridePct+"% off high":"—")))+"</td>"+
