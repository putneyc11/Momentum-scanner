...
<div class="card">
  <div class="bar"><span class="t">Models · seven strategies, one book — nightly ensemble tune re-splits risk</span></div>
  <div style="overflow-x:auto"><table id="mtbl"><thead><tr>
    <th>Model</th><th>Risk weight</th><th>Open</th><th>Day P/L</th></tr></thead><tbody></tbody></table></div>
</div>
<div class="card">
  ...
  <div style="overflow-x:auto"><table id="ptbl"><thead><tr>
    <th>Sym</th><th>Model</th><th>Qty</th><th>Entry</th><th>Now</th><th>Planned exit</th><th>Stop loss</th><th>Chg %</th><th>Value</th><th>P/L $ / %</th></tr></thead><tbody></tbody></table></div>
...
<div class="card">
  ...
  <div style="overflow-x:auto"><table id="tbl"><thead><tr>
    <th>Time (ET)</th><th>Event</th><th>Sym</th><th>Model</th><th>Detail</th><th>P/L</th></tr></thead><tbody></tbody></table></div>
...
function render(){
  ...
  $("noPos").textContent=(st.session==="closed")
    ...
    : "No open trades right now — "+((st.strats||[]).length||7)+" models watching "+(st.universe||0)+" scanner movers for their setups (entries 4:00–19:30 ET).";
  const mtb=document.querySelector("#mtbl tbody");mtb.innerHTML="";
  const today2=etDay(Date.now());
  for(const m of (st.strats||[])){
    const pl=(D.trades||[]).filter(j=>j.strat===m.key&&j.pnl!=null&&etDay(j.t)===today2).reduce((a,j)=>a+j.pnl,0);
    const tr=document.createElement("tr");
    tr.innerHTML="<td class=sym>"+m.name+"</td><td>"+(m.weight!=null?"×"+Number(m.weight).toFixed(2):"—")+"</td>"+
      "<td>"+(m.open||0)+"</td><td class='"+(pl>=0?"pos":"neg")+"'>"+(pl>=0?"+$":"−$")+Math.abs(pl).toFixed(2)+"</td>";
    mtb.appendChild(tr);
  }
  for(const p of poss){
    ...
    const tr=document.createElement("tr");
    tr.innerHTML="<td class=sym>"+p.sym+"</td><td>"+(p.strat||"—")+"</td><td>"+(p.qty!=null?p.qty:"")+"</td>"+
      "<td>$"+Number(p.entry).toFixed(2)+"</td><td>$"+Number(p.price).toFixed(2)+"</td>"+
      "<td>"+(p.exiting?"closing — sell working":p.scaled?"runner · trailing":(p.target!=null?"$"+Number(p.target).toFixed(2)+" · sell "+(p.scaleOutPct||85)+"%":(p.ridePct?"ride · exit −"+p.ridePct+"% off high":"—")))+"</td>"+
  ...
  for(const j of rows){
    ...
    tr.innerHTML="<td>"+fETd(j.t)+" "+fET(j.t)+"</td><td>"+(j.kind==="entry"?"▲ entry":j.kind==="scale"?"◆ scale-out":"exit · "+(j.reason||""))+
      "</td><td class=sym>"+(j.sym||"")+"</td><td>"+(j.strat||"—")+"</td><td>"+(j.kind==="entry"?("x"+j.qty+" @ $"+Number(j.px).toFixed(2)+" stop $"+Number(j.stop).toFixed(2)):"")+
