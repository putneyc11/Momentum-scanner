...
function startDashboard({ port, stateDir, status }) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (!authed(req)) { res.writeHead(401, { "Content-Type": "text/plain" }); res.end("add ?token=... (DASH_TOKEN env var)"); return; }
    ...
    if (u === "/api/state") {
      ...
      res.end(JSON.stringify({
        ...
        trades: tailJsonl(journalFile, 200).filter((j) => j.kind === "entry" || j.kind === "exit"),
        status: status || {},
        now: Date.now(),
      }));
      return;
...
<div class="card">
  <div class="bar"><span class="t">Equity curve · ▲ entry · ● win exit · ✕ loss exit</span>
...
<div class="note">Samples every ~30s while the engine runs (4:00–16:00 ET). Paper account only. Not investment advice.</div>
</div>
<script>
...
function render(){
  ...
  const st=D.status||{};
  $("posN").textContent=st.positions!=null?st.positions.length:"—";
  $("posSyms").textContent=st.positions&&st.positions.length?st.positions.join(" "):"flat";
  ...
  for(const j of rows){
    ...
    tr.innerHTML="<td>"+fETd(j.t)+" "+fET(j.t)+"</td><td>"+(j.kind==="entry"?"▲ entry":"exit · "+(j.reason||""))+
