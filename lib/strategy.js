...
/* Tuner search space: [min, max, step]. Anything not listed is fixed.
   ...
   round trips and a flattering number with nothing underneath. engine.js:93
   promotes on the validate score alone, unattended, every night — handing that
   promoter a wider box on a pod with no edge is how a hollow champion reaches
   live trading. The box gets widened per pod, on evidence, one pod at a time. */
const RANGES = {
  minConfluence: [2, 5, 1],
