...
/* FORCED-EXIT SLIPPAGE CEILING (HYP-005). The participation cap now applies to
   exits too, so an exit larger than the bar's capacity unwinds over several
   bars. One exit cannot do that: the end-of-day residue, because "no overnight
   exposure" is a real constraint and not a modelling convenience. That residue
   is forced, and it pays slippage scaled by how many times the cap it is
   demanding -- sweeping five times the available tape costs five times the
   spread.

   Unbounded, that product goes negative: the worst observed exit demanded 26x
   the whole minute's volume, which at 100 bps base would price the fill below
   zero. So it is clamped here.

   THIS NUMBER IS A BOUND ON A FICTION, NOT A MEASUREMENT. Nothing establishes
   that 20% is what it would actually cost to dump size into after-hours tape
   on a $2 name; it is chosen to keep a forced fill finite and pessimistic. The
   honest handling is that every forced residue is tagged in the trade record
   and reported separately, so a reviewer can see exactly how much of any result
   rests on it rather than having it silently averaged in.

   Frozen and absent from RANGES, like the participation cap: a tuner given
   this knob would optimise its way out of paying for size. */
const MAX_FORCED_SLIP_BPS = 2000;
...
module.exports = { DEFAULTS, RANGES, MAX_BAR_PARTICIPATION, MAX_FORCED_SLIP_BPS, SLIP_TIERS, slipBpsFor, prepSeries, signalAt, exitCheck, entryViable };
