...
/* Tuner search space: [min, max, step]. Anything not listed is fixed.
   ...
   STOP BOUNDS (widened 2026-08-28, HYP-002). The three stop knobs used to be
   capped at stopAtrMult 3 / minStopPct 4 / maxStopPct 12. Nobody ever tested
   that box; it was an assumption. A sweep over the full 144-day library under
   the tiered slippage model (3aadaea) found redgreen's profit factor still
   climbing at the far corner — 0.70 at the old ceiling, 1.16 at 12 / 30 / 90.
   The whole improving region sat OUTSIDE the search space, so the nightly
   tuner could not reach it. On a $1-5 name a 1-4% stop is about one bar of
   ordinary range, which is why 83% of redgreen's trades exited on the stop
   rather than on the drift the entry was selecting for.

   The ceilings here sit ABOVE the best point measured, deliberately: a bound
   placed exactly at the best tested value pins the optimum to the wall and you
   learn nothing about what is past it. maxStopPct stops short of 100 because
   at 100 the stop is at zero, which is not a wide stop, it is no stop.

   Cost of the wider box: at a fixed iteration count the search is less dense
   per unit of parameter space. That is the trade being made. */
const RANGES = {
  minConfluence: [2, 5, 1],
