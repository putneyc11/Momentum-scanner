...
/* ---- 6. First Expansion (surge) ----
   HYP-003. igniterSignal waits for three green bars totalling >= burstPct with a
   volume surge, so it fires on the LAST print of a squeeze: its +5m forward
   return gets worse as the burst gets bigger (-61.7 bps on 6%+ moves, which are
   7,868 of its 14,093 hits). This buys the same volume event at its start --
   the first expansion bar, before the run it is riding has happened.

   igniterSignal is deliberately untouched: igniter still uses it. */

/* Pinned, NOT P.rsiMax. surge's rsiMax default is 90 and its RANGES search it
   up to 95; this entry was measured with RSI held at 80 and the number does not
   transfer to a looser bound, so the tuner must not be able to move it. */
const FIRST_EXP_RSI_MAX = 80;
const FIRST_EXP_LOOKBACK = 5;      /* bars whose high the close must clear */
const FIRST_EXP_VWAP_PCT = 0.02;   /* open may sit at most this far ABOVE vwap */

function firstExpSignal(S, bars, i, P) {
  const b = bars[i];
  if (!inWindow(b, P) || i < 10 || S.atr[i] == null) return null;
  const c = b.c;
  if (!(c > b.o)) return null;                                    // green
  if (!(S.avg10[i] && b.v >= P.volSurgeMult * S.avg10[i])) return null;
  if (!(c > S.vwap[i])) return null;
  if (S.rsi[i] != null && S.rsi[i] >= FIRST_EXP_RSI_MAX) return null;
  /* FIRST bar of the expansion: if the prior bar was itself a green volume
     surge, this one is a continuation and the move has already started. */
  const p = bars[i - 1];
  if (p.c > p.o && S.avg10[i - 1] && p.v >= P.volSurgeMult * S.avg10[i - 1]) return null;
  let hi = -Infinity;
  for (let k = i - FIRST_EXP_LOOKBACK; k < i; k++) hi = Math.max(hi, bars[k].h);
  if (!(c > hi)) return null;                                     // breaks the 5-bar high
  /* still near value. One-sided by design: an open BELOW vwap is fine, an open
     already extended above it is the thing being excluded. */
  if (!(S.vwap[i] > 0) || (b.o - S.vwap[i]) / S.vwap[i] > FIRST_EXP_VWAP_PCT) return null;
  const dist = stopDist(c, S.atr[i], P, c - b.l);
  return dist ? { stop: c - dist, risk: dist } : null;
}
...
const STRATS = [
  ...
  },
  {
    ...
    /* burstPct and rsiMax are now DEAD KNOBS for this pod: firstExpSignal reads
       neither (RSI is pinned at FIRST_EXP_RSI_MAX). HYP-003 says leave RANGES
       alone, so they stay and the tuner wastes draws on them. Worth its own
       change once this entry is graded. */
