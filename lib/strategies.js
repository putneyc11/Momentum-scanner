...
const MGMT = {
  ...
  reentryLimit: 4, cooldownMin: 5,
};
/* The shared stop box stays where it was. Widening it here would reach flag,
   igniter and gapgo as well; see WIDE_STOP_RANGES below for why that is not
   wanted, and lib/strategy.js#RANGES for the same note on gapgo. */
const MGMT_RANGES = {
...
/* HYP-002: the WIDE STOP BOX, applied per pod and only where it was measured.

   WHAT MOTIVATED THIS IS CONTAMINATED. Read that first. A sweep in
   research/stops2.js showed redgreen climbing from 0.70 profit factor at the
   old ceiling to 1.16 at 12 / 30 / 90. Red Team killed it (REVIEWS/HYP-001.md):
   the sweep scored all six stop triples on the full 144 days and both halves,
   then the best row was picked, so every day later called "test" had already
   been seen. It is an in-sample, post-selection number, not an estimate of
   anything out of sample. The entry-edge figure that framed it came from
   research/edge.js, which drew its control uniformly across 09:30-19:30 while
   6,191 of redgreen's 13,870 signals fire in the 09:00 hour; controlled to the
   same 15-minute bucket the signal scores WORSE than its control.

   So this table is not evidence and nothing here should be read as a claim
   that a wide stop works:

     stopAtr/min%/max%      all       H1       H2    stop-outs
     1.5/1/4  (default)    0.70     0.69     0.72         83%
     12/30/90              1.16     1.19     1.13         11%

   What survives is narrower and worth testing on its own: the tuner's stop
   ceiling was never chosen on evidence, and the region above it was never
   searched. Whether anything is there is decided by the A/B tune in
   HYP-002 -- same pods, seeds, cost model and days, old bounds against new,
   with a frozen holdout that no search has read. That is the only clean
   measurement on this question and it is the one to believe.

   And if it does win, note what it would mean. Raising maxStopPct only lifts a
   CAP; the minStopPct floor and the ATR distance still set the stop. At
   12 / 30 / 90, 87.5% of exits are VWAP or time exits rather than stops, so
   the configuration is not "redgreen with more noise tolerance" -- it is
   redgreen with signal-based exits. A win here is a result about exit policy
   and should be reported as one.

   Ceilings sit above the swept corner on purpose: a bound placed exactly at
   the best tested value pins the optimum to the wall. maxStopPct stops at 95
   rather than 100 because at 100 the stop is at zero, which is not a wide
   stop, it is no stop.

   SPREAD THIS INTO A POD ONLY WITH THAT POD'S OWN CLEAN MEASUREMENT IN HAND.
   It is deliberately not in MGMT_RANGES: a wider box on a pod with no entry
   edge yields a flattering champion with nothing underneath it.

   Known dead region: this box overlaps MGMT_RANGES' maxStopPct, so the tuner
   can draw minStopPct 40 alongside maxStopPct 4. stopDist applies the max clamp
   last, so the clamp wins and that pair behaves as a 4% stop. Those draws score
   like the narrow config that already loses, so they are wasted iterations
   rather than a trap that captures the optimum -- the search is handicapped,
   not misled. Pinned by test rather than fixed; rejecting the pair is an
   exit-engine change that would move every pod's numbers. */
const WIDE_STOP_RANGES = {
  stopAtrMult: [0.8, 15, 0.1],
  minStopPct: [1, 40, 0.5],
  maxStopPct: [4, 95, 1],
};
...
const RIDE = {
  ...
  vwapExit: 0,
  /* STALL EXIT: a rider below +0.5R after two hours is a dead moonshot —
     recycle the slot. A working runner (above +0.5R) is NEVER touched by
     this; it can ride all day. */
  timeStopMin: 120,
...
const STRATS = [
  ...
  },
  {
    ...
    RANGES: { ...MGMT_RANGES, ...WIDE_STOP_RANGES, dipBars: [1, 4, 1], rsiMax: [60, 85, 5] },
  ...
  },
  {
    ...
    RANGES: { ...MGMT_RANGES, ...WIDE_STOP_RANGES, entryStartMin: [570, 660, 15], rsiMax: [60, 90, 5] },
    signalAt: redGreenSignal,
  },
];

module.exports = { STRATS, MGMT, MGMT_RANGES, WIDE_STOP_RANGES };
