...
/* Stop bounds widened 2026-08-28 (HYP-002) to match lib/strategy.js#RANGES —
   the old 3 / 4 / 12 ceilings excluded the entire region where profit factor
   was still improving, so the nightly tuner searched a box that did not
   contain the answer. See the long note above RANGES in lib/strategy.js.

   These three keys are read by every QUICK-STRIKE pod that spreads
   MGMT_RANGES (reclaim, flag, igniter, redgreen), so this widens four search
   spaces, not one. It changes no pod's behaviour on its own — a range is only
   a search bound, and saved champions are untouched until a pod is retuned. */
const MGMT_RANGES = {
