...
/* HYP-002: the WIDE STOP BOX, applied per pod and only where it was measured.
   ...
   Under tiered slippage, redgreen over the full 144-day library:

     stopAtr/min%/max%      all       H1       H2    stop-outs
     1.5/1/4  (default)    0.70     0.69     0.72         83%
     8/12/40               1.03     1.13     0.94         28%
     10/20/60              1.06     1.12     1.02         17%
     12/30/90              1.16     1.19     1.13         11%

   The old ceiling is the top row, so the entire region above 1.0 sat outside
   the box the nightly tuner searched. On a $1-5 name a 1-4% stop is about one
   bar of ordinary range, and the position is stopped out by noise before the
   drift its entry selects for has time to appear.

   Ceilings sit ABOVE the measured corner on purpose: a bound placed exactly at
   the best tested value pins the optimum to the wall and teaches you nothing
   about what is past it. maxStopPct stops at 95 rather than 100 because at 100
   the stop is at zero, which is not a wide stop, it is no stop.

   SPREAD THIS INTO A POD ONLY WITH THAT POD'S OWN SWEEP IN HAND. It is not in
   MGMT_RANGES for a reason: engine.js:93 promotes on the validate score alone,
   unattended, every night, and a wider box on a pod with no entry edge yields a
   flattering champion with nothing underneath it.
   ...
   rather than a trap that captures the optimum — the search is handicapped, not
   misled. Pinned by test rather than fixed; rejecting the pair is an exit-engine
   change that would move every pod's numbers. */
