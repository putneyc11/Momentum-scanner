...
const { prepSeries, signalAt, exitCheck, entryViable, MAX_BAR_PARTICIPATION, slipBpsFor } = require("./strategy");
...
function runDay(day, P, equity, signalFn = signalAt) {
  ...
  const closeOut = (sym, pos, price, reason, m) => {
    const px = reason === "target" ? price : price * (1 - slipBpsFor(price, P) / 1e4);
