...
const { prepSeries, signalAt, exitCheck } = require("./strategy");

function runDay(day, P, equity) {
  ...
  for (let m = 4 * 60; m < 16 * 60; m++) {
    ...
    for (const sym of Object.keys(open)) {
      ...
      if (ex) closeOut(sym, pos, ex.price != null ? ex.price : d.bars[i].c, ex.reason, m);
    }
    ...
    for (const sym of Object.keys(pending)) {
      ...
      if (qty < 1) continue;
      open[sym] = { fill, stop: sig.stop, risk, qty, hwm: fill, barsHeld: 0, entryM: m, entry: fill };
    ...
    if (!halted && m < P.flattenMin) {
      for (const sym of Object.keys(data)) {
        ...
        const i = d.idx.get(m);
        if (i == null) continue;
        const sig = signalAt(d.S, d.bars, i, P);
        if (sig) pending[sym] = sig;
      }
    }
  }
  ...
  for (const sym of Object.keys(open)) {
    const d = data[sym];
    closeOut(sym, open[sym], d.bars[d.bars.length - 1].c, "eod", 16 * 60);
  }
  return { equity, trades };
}

function runBacktest(days, P, startEq = 100000) {
  let equity = startEq;
  const trades = [];
  const curve = [equity];
  for (const day of days) {
    const r = runDay(day, P, equity);
    equity = r.equity;
